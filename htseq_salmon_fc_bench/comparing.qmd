...
2.  Отфильтровать по каунтам HTSEQ (есть) 2.1 Выкинуть образцы HTSEQ из FC и salmon (сделаем на этапе корреляции) 2.2 Фильтровать по отсечке для каждого типа подсчета каунтов
3.  Сравнить полученные матрицы сырых ридов между собой 3.1 Пирсоновская корреляция на общем датасете (без нулей везде) 3.2 Пирсоновская корреляция на датасете, с максимальным удалением образцов
4.  Посмотреть на ДЭ, сравнить разные эксперименты (направление ДЭ, логфолчендж для разных генов)

> TODO Фильтрация как в статье DE пан-тишью и потканевая Меры отличия: - количество выпадающих образцов (фильтр по образцам) - количество выпадающих генов (фильтр по генам и спецфильтр по генам) - дифэкспрессия
...
# Потканевая функция
de_2_fit <- function(edge_df, group_vector1, group_vector2){
  # Двоеточие или плюс?
...
salmon_de_fit <- de_simple_fit(tmm_salmon_genes, salmon_ages)
...
Какая-то хрень с 6-м месяцем, очень высокмй lfc
...
Приведение айдишников к единому виду
...
# pre_cor_raw_counts_df <- pre_cor_raw_counts_df_with_meta
...
### Корреляция между TPM

```{r}
tpm_htseq <- read_csv("tpm_htseq.csv")
tpm_salmon <- read_csv("tpm_salmon_gene.csv")

common_genes <- intersect(tpm_salmon$feature, tpm_htseq$ALIAS)
common_samples <- intersect(colnames(tpm_htseq), colnames(tpm_salmon))

pre_cor_TPMs_df <- inner_join(
  tpm_htseq |>
    select(ALIAS, common_samples) |> 
    filter(ALIAS %in% common_genes) |> 
    distinct(ALIAS, .keep_all = TRUE) |> 
    rename(feature = "ALIAS") |> 
    pivot_longer(!feature, names_to = "Sample name", values_to = "TPM_htseq"),
  tpm_salmon |> 
    select(common_samples) |>
    filter(feature %in% common_genes) |> 
    pivot_longer(!feature, names_to = "Sample name", values_to = "TPM_salmon"),
  by = "feature"
)
